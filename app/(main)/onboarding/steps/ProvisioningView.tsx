...
export function ProvisioningView({ assistantName = "" }: { assistantName?: string }) {
  ...
  // A guest (dev bypass, no session) or an account with no saved funnel can't be
  // provisioned - there's no user to attach a number to. Rather than hang on the
  // loader, send them to the dashboard, matching the dev shortcut's old target.
  useEffect(() => {
    if (state.status === "guest" || state.status === "nothing") {
  ...
  // One continuous reveal: the logo bars breathe while provisioning, then part
  // and the assistant pops in to say hello the moment it's live.
  const done = state.status === "done";
  const waiting = state.status === "waiting-payment";
  return (
    <section className="flex flex-col items-center py-16 text-center" aria-busy={!done}>
      <AssistantReveal ready={done} />
      {done ? (
