...
import type { VoiceOption } from "@/app/(main)/dashboard/numbers/voices";
...
export function MeetAssistant({
  ...
  // Pre-typed from the saved config so a returning caller sees the company name
  // they already entered, exactly like every other field on this step. The
  // anti-autofill attributes on the input below still stop the browser from
  // injecting a remembered value; this only restores the user's own saved input.
  const [business, setBusiness] = useState(config.companyName ?? "");
  // Company website - an alternative to the company name for this substep
  // (either one advances). Scraped into the assistant's knowledge after payment,
  // during provisioning; persists across back/forward like every other field.
  ...
  // Either a company name or a website satisfies the first substep: the site is
                  ...
                  {step === 0 && (
                    <Question label={o.qBusiness}>
                      <input
                        ...
                        className={field}
                      />
                      {/* Either identifier is enough. The website is the richer
                          signal - it's scraped into the assistant's knowledge
                          after payment, so step 2 only adds PDFs/notes on top. */}
