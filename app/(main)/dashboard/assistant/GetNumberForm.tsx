...
type PoolCountry = { code: string; name: string; flag: string; count: number };

interface Props {
  assistantId: string;
  credits: number;
  availableCount?: number;
  /** Countries with numbers available in the pool, for the assign picker. */
  poolCountries?: PoolCountry[];
}

export function GetNumberForm({ assistantId, credits, availableCount = 0, poolCountries = [] }: Props) {
  const t = useT();
  const [code, setCode] = useState(DEFAULT_COUNTRY);
  const [poolCode, setPoolCode] = useState(
    poolCountries.find((c) => c.code === DEFAULT_COUNTRY)?.code ??
      poolCountries[0]?.code ??
      DEFAULT_COUNTRY,
  );
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          {poolCountries.length > 0 && (
            <div className="flex-1">
              <label htmlFor="pool-country" className="mb-1.5 block text-sm font-medium text-neutral-700">
          ...
          <span className="flex shrink-0 items-center gap-2 sm:pb-0.5">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 divide-y divide-neutral-200/70 overflow-hidden rounded-lg border border-neutral-200/70 bg-white/60 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
