"use client";

import { useFormStatus } from "react-dom";
// app/components/, not app/dashboard/components/ - this path is depth-2 by
// design.
import { MathLoader } from "../../components/MathLoader";

export function TestCallButton({
  action,
  className,
  idleLabel,
  pendingLabel,
  disabled = false,
  title,
}: {
  action: (formData: FormData) => void | Promise<void>;
  className?: string;
  idleLabel: string;
  pendingLabel: string;
  /** With no transfer number set there is nowhere to ring. */
  disabled?: boolean;
  title?: string;
}) {
  const { pending } = useFormStatus();
  return (
    <button
      type="submit"
      formAction={action}
      disabled={pending || disabled}
      title={title}
      className={`press ${className ?? ""}`}
    >
      {pending ? (
        <MathLoader variant="spiro" size={16} label={pendingLabel} />
      ) : (
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
          <path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02L6.62 10.79z" />
        </svg>
      )}
      {pending ? pendingLabel : idleLabel}
    </button>
  );
}
