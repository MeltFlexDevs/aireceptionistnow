  ...
  return (
    <form action={formAction} className="flex shrink-0 items-center gap-2">
      ...
      <button
        type="submit"
        disabled={pending}
        className="press rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-xs font-medium text-neutral-600 transition-colors hover:border-rose-200 hover:text-rose-600 disabled:cursor-wait disabled:opacity-60"
      >
          ...
          <NotesModal
            ...
            trigger={(open) => (
              <button
                type="button"
                onClick={open}
                className="press shrink-0 rounded-lg border border-neutral-200 bg-white px-2.5 py-1 text-xs font-medium text-neutral-700 hover:bg-neutral-50"
              >
                {t.common.edit}
              </button>
            )}
